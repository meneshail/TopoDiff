...
def get_pdb_headers(prot: Protein, chain_id: int = 0) -> Sequence[str]:
    ...
    if prot.label_override is not None:
        pdb_headers.append(f"{prot.label_override}")
        remark = prot.remark
...
def from_nparray(
    ...
    """Assembles a protein from numpy arrays.

    Args:
      atom_positions: Atom positions.
      atom_mask: Atom mask.
      aatype: (Optional) Amino acid types.
      residue_index: (Optional) Residue indices.
      b_factors: (Optional) B-factors.
      chain_index: (Optional) Chain indices for multi-chain predictions
      remark: (Optional) Remark about the prediction
      parents: (Optional) List of template names
    Returns:
    ...
    """
    if aatype is None:
        aatype = np.ones_like(atom_mask, dtype=np.int32) * 20
    if residue_index is None:
        residue_index = np.arange(atom_mask.shape[0]) + 1
    if b_factors is None:
        b_factors = np.zeros_like(atom_mask)
    if b_factors.ndim == 1:
        b_factors = b_factors[:, None].repeat(atom_mask.shape[1], axis=1)
